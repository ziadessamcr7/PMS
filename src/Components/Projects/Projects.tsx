...
export default function Projects() {
  ...
  const updateProject = (data: any) => {
    ...
    axios
      .put(`${BaseUrl}/Project/${projId}`, data, {
        headers: requestHeaders,
      })
      .then((response) => {
        handleClose();
    ...
    <section className="">
      ...
      <div className="d-flex justify-content-between bg-white rounded-2 p-3">
        ...
        <Link
          ...
          className="btn rounded-5 text-white d-flex align-items-center fw-bold"
              ...
              {projectList?.map((proj: any, idx: any) => {
                return (
                  <tr key={idx}>
                    ...
                    <td>{proj?.id}</td>
                    <td>{proj?.manager?.userName}</td>

                    <td>
                      <Dropdown>
                        <Dropdown.Toggle
                          style={{ backgroundColor: "rgba(49, 89, 81, 0.90)" }}
                          id="dropdown-basic"
                        ></Dropdown.Toggle>
